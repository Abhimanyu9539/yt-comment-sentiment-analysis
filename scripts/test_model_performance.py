...
def test_model_performance(model_name, stage, holdout_data_path, vectorizer_path):
    try:
        ...
        X_holdout_raw = holdout_data.iloc[
            :, :-1
        ].squeeze()  # Raw text features (assuming text is in the first column)
        ...
        X_holdout_tfidf_df = pd.DataFrame(
            X_holdout_tfidf.toarray(), columns=vectorizer.get_feature_names_out()
        )
        ...
        precision_new = precision_score(
            y_holdout, y_pred_new, average="weighted", zero_division=1
        )
